...
measure_images = tf.concat(0, [image[:,:,:,0:3] for image in [all_images[i] for i in [0, 2, 4, 8]]])
...
with tf.Session() as sess:
  ...
  for i in range(1000000):
     ...
     if not (i%10):
       summary_str, out, ae, ge, gg, dif, show_images_res = sess.run([merged_summary_op, adv_inp, adv_entropy, gen_entropy, al, grad_op, show_images], feed_dict={})
     ...
     ae, ge, _ = sess.run([adv_entropy, gen_entropy, chosen_step])
     print i, ae, ge
